...
namespace Spells
{
    public class Bite_EnemySpell : SpellObject
    {
        [SerializeField]
        public string biteAssetName, sfxName;
        [SerializeField] private StatusEffectBase bleedEffect, poisonEffect;

        protected override IEnumerator OnActivate()
        {

            //Please review if theres anything that needs to be changed which most likely there will be since this is basically the lightning one 
            ...
            AssetHelper.PlayEffect(biteAssetName,target.transform.position,Quaternion.Euler(15f,0f,0f));
            ...
            {
                var _bleedInstance = Instantiate(bleedEffect, Vector3.zero, Quaternion.identity);
                ...
                var _poisonInstance = Instantiate(poisonEffect, Vector3.zero, Quaternion.identity);
                var _damage = Mathf.RoundToInt(character.stats.strength * 0.1f);
                
                _damage = Mathf.Clamp(_bleedDmg, 1, 9999);

                if (_poisonInstance != null)
                {
                    _poisonInstance.SetDamage(_damage);
                    yield return target.character.statusEffectReceiver.ApplyStatusEffect(_poisonInstance, character.gameObject);
                }
            }
